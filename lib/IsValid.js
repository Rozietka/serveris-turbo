class IsValid {
    ...
    static email(text) {
        ...
        if (parts.length !== 2) {
            return [true, '"Email" turi tureti 1 @ simboli'];
        }
        if (parts[0] === '') {
            return [true, '"Email" truksta dalies pries @ simboli'];
        }
        if (parts[1] === '') {
            return [true, '"Email" truksta dalies uz @ simboli'];
        }
        const domainParts = parts[1].split('.');
        if (domainParts.length !== 2
            || domainParts[0] === ''
            || domainParts[1] === '') {
            return [true, '"Email" domeno dalis turi buti validus website adresas'];
        }
