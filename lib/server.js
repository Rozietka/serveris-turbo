...
server.httpServer = http.createServer((req, res) => {
    ...
    req.on('end', async () => {
        ...
        if (isPage) {
            ...
            // tikriname koks pirmo lygio psl tiktu
            let pageClass = routeList[level1] ? routeList[level1] : routeList['404'];

            // tikriname koks antro lygio blog psl tiktu
            if (level1 === 'blog' && level2) {
                const [postErr, postContent] = await file.read('blog', level2 + '.json');

                pageClass = PageBlogPost;
            }
