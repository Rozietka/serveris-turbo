...
server.httpServer = http.createServer((req, res) => {
    ...
    req.on('end', async () => {
        ...
        if (isPage) {
            ...
            // atsirenkam koki route list naudosim(public/protected)
            // trimmped path split -> [lvl1, lvl2, ...., lvln]

            // jeigu LVL count === 1
            // perduodame dataForHandlers
            // susirandame reikia psl klase
            // ja inicijuojame
            // render
            // graziname HTML
            ...
            // jeigu LVL count >= 2
            // perduodame dataForHandlers
            // patikriname, ar toks 2 lvl psl egzistuoja, t.y. ar yra straipsnis.json failas
            // susirandame reikia psl klase: blog, services, team...
            // ja inicijuojame (ir perduodam papildomus duomenis???)
            // render
            // graziname HTML

            const routeList = dataForHandlers.user.isLoggedIn ? server.protectedRoutes : server.publicRoutes;
            const [level1, level2, level3] = trimmedPath.split('/');
