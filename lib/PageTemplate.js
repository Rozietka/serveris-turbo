...
class PageTemplate {
    ...
    headHTML() {
        return `<head>
                    ...
                    <base href="http://localhost:${config.httpPort}/">
    ...
    async render() {
        ...
        if (this.isLogoutPage) {
            const tokenID = this.data.cookies['login-token'];
            const logoutCookie = [
                'login-token=' + tokenID,
                'path=/',
                'domain=localhost',
            ...
            headersObj['Set-Cookie'] = logoutCookie.join('; ');

            await file.delete('token', tokenID + '.json');
        }
